...
void sweepPolygonAxis(Grid* grid, Polygon polygon, size_t axis, char pixel) {
    ...
    for (size_t i = 0;
         i < polygon.size; i++) {                                            // Iterating over pairs of polygon points,
        if (polygon.vertices[i].j == axis) {
            if (polygon.vertices[j].j ==
                axis) {                                        // if they are vertically aligned,
                if (polygon.vertices[i].i <
                    polygon.vertices[j].i) {                    // plot the corresponding line directly;
                ...
                grid->matrix[llrintf(
                        polygon.vertices[i].i)][axis] = pixel;                // so plot it directly as well;
            }
        }

        if ((polygon.vertices[i].j <= axis && axis < polygon.vertices[j].j)                // in any case,
            || (polygon.vertices[j].j <= axis &&
                axis < polygon.vertices[i].j)) {    // if the polygon side intersects with the axis,
            insertValue(intersOrds, lastPointIndex,
                        polygon.vertices[i].i +
                        (axis - polygon.vertices[i].j)    // insert its ordinate in a sorted manner;
    ...
    if (lastPointIndex != 0UL) {
        for (size_t i = 0;
             i < lastPointIndex - 1; i += 2) {                            // finally, fill the polygon vertically.
